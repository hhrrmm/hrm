...
/*		public function clone_deep(source:ArrayCollection):ArrayCollection {
		...
		public function change_freq(source:Array, init_freq:Number, rez_freq:Number, agg_type:Number, HRunits:Array = null):Array {
			var a:Array = new Array();
			//if ((init_freq == 1)&&(rez_freq == 3)) {
			if (init_freq == 1) {
				switch (agg_type) {
					case 1: //sum agg_type == 1
						a = aggregate(source, agg_type, rez_freq);
						break;
					...
					case 3: //CPI  agg_type == 3
						...
						b = aggregate(source, agg_type, rez_freq);
						a = mult(b, 100); 
						break;
					case 4: //prices
						a = aggregate(source, agg_type, rez_freq, HRunits);
						break;
					default:
						break;
				};
		...
		public function change_freq_2(source:Array, init_freq:Number, rez_freq:Number, agg_type:Number, agg_type_right:Number,  HRunits:Array = null):Array {
			...
			if (init_freq == 1) {						
					a = aggregate(source, agg_type, rez_freq, HRunits);
					//extract second indicator form source
					...
					if (agg_type_right > 0) {
						if (agg_type_right == 3) {
							var b:Array = new Array();
							b = aggregate(b0, agg_type_right, rez_freq);
							a2 = mult(b, 100);
						} else {
							a2 = aggregate(b0, agg_type_right, rez_freq, HRunits);
						};
					} else {
						//labels????
		...
		public function aggregate(source:Array, agg_type:uint, rez_freq:uint, HRunits:Array = null):Array { 
			var indConst:int = 1;
			var const_divider:Number = 1.209381942; // base - year - 1983
			//1.24015774665188 - for quarter
			var initial_level:Number = 1.30677259178366; //level of CPI at 1985 for initial values, 
			...
			if (rez_freq == 3) { //annual
				...
				if ((source[12]['period']).toString().substr(0, 4) == '1981') {
					initial_level = 1;
				};
			...
			for (var f:uint = indConst; f < source.length; f++) {
