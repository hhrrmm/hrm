...
$inds15 = $f->calculateForecasts($first_timeID, $last_timeID, false, true);
...
if ($inds15 == "DB_Error") {	
	...
	for ($j = $first_timeID-12; $j < $last_timeID; $j++) {
		if ($inds13[$j] == "") {$inds14[$j] = "";}
		else {$inds14[$j] = $inds13[$j]*$inds11[$j];}
...
$update_result = updateTable($inds14, $inds15, $inds16, $timeIDs, $first_timeID, $last_timeID, 0, 0);
